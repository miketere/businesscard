...
export default function FashionPinkTemplate({ cardData }: { cardData: CardData }) {
  return (
    <div className="w-full max-w-sm mx-auto aspect-[9/16] bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col">
      <div className="h-24 relative" style={{ background: `linear-gradient(135deg, ${cardData.primaryColor}, ${cardData.secondaryColor})` }}>
      ...
      <div className="px-6 pb-6 -mt-6 flex-shrink-0">
        {cardData.profileImage ? (
          <img src={cardData.profileImage} alt={cardData.name} className="w-32 h-32 rounded-full mx-auto mb-4 object-cover border-4 border-white shadow-xl" />
        ) : (
          <div className="w-32 h-32 rounded-full mx-auto mb-4 border-4 border-white shadow-xl" style={{ backgroundColor: cardData.primaryColor }}></div>
        )}
